...
class AStarOfTheShow(CaptureAgent):
    class Mode: #helper class to select mode
        Attack = 0
        Defend = 1
        Retreat = 2
        Capsule = 4
    ...
    def DecisionTree(self,game_state):
        ...
        if Agent1 == self.index:
            ...
            if (self.initial_time-game_state.data.timeleft) < 150:        
                return self.Mode.Retreat        #get into position
            
            if  Agent1_state.scared_timer > 0 or not invaders:
                return self.Mode.Attack     #while you are scared try to atack for value 1
            
            if carring  >= 2  and self.scared_time_remaining(game_state) < 10 or (game_state.data.timeleft < 200 and carring > 1):
                return self.Mode.Retreat   #get back home 
            return self.Mode.Defend #default defend
    ...
    def heuristic_defend(self, game_state, action):
        ...
        if self.red:
            if position[0] < self.width_half:
                my_score += 10      #don't leave your side
        else: 
            if position[0] > self.width_half:
                my_score += 10      #don't leave your side            
        return my_score
